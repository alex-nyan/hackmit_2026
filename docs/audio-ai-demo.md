...
Pre-merge review retained the exact source/capture-matched GPS annotation when a phrase alert replaces its contextual report on the map, kept urgent contextual reports available for acknowledgment beyond the six most recent assessments, and preserved the model alert fallback if its separate phrase event is absent. After rebasing onto the latest landing-page changes, typecheck and changed-file lint passed. A fresh manual bomb/explosion request produced an urgent phrase alert followed by an urgent Ollama assessment in 1.1 seconds; the browser displayed both, and acknowledgment removed that synthetic alert from the pending list. Older urgent reports remained visible beyond the six newest assessments. Physical GPS enrichment was not exercised; no tests were added or suite run.
