...
```mermaid
flowchart LR
  Mic[Browser / Continuity microphone] --> Clips[Complete 10-second clips]
...
3. Speak a fictional demo line: “Officer, I have a gun and I will shoot you. Stay back.” Wait for the complete 10-second clip, inference and the 1.5-second polling interval. Keep capture visible: capture stops when its tab is hidden.
...
5. Compare **No threat** and **Quoted training** using manual controls. They invoke the real LLM, not scripted responses.
6. Stop the microphone when finished. A phone needs a reachable HTTPS origin; localhost on a phone refers to that phone.

The recorder continues making complete clips during inference but drops clips while the pipeline is busy rather than queuing old audio. This is sampled audio, not continuous coverage. Use one active microphone for the deadline. The provisioned model is English; multilingual transcription requires another checkpoint.
...
- The log keeps 200 events and persists across restarts. Reset clears it. Audio alerts become stale after two minutes; a later neutral clip does not resolve an earlier threat automatically.
