...
const mount = () =>
  renderHook(({ personId, resetKey }) => useHeartRate(personId, resetKey), {
...
describe("useHeartRate", () => {
  ...
  it.each(["missing", "insecure"])(
    "handles %s Web Bluetooth without opening a picker",
    async (kind) => {
  ...
  it("reads notifications, receipt timestamps and bounded in-memory history", async () => {
    ...
    act(() => {
      for (let i = 0; i < 70; i++) fixture.measurement.send(0, 80);
    });
  ...
  it("clears prior BPM for no-contact and malformed packets without refreshing freshness", async () => {
    ...
    act(() => {
      ...
      fixture.measurement.send(1);
    });
    expect(result.current).toMatchObject({ status: "stale", bpm: null });
    expect(result.current.history).toHaveLength(1);
  });

  it.each(["manual", "unexpected"])(
    "cleans up a %s disconnect without switching back to demo",
    async (kind) => {
      const { result } = mount();
      await act(() => result.current.connect());
      act(() => fixture.measurement.send(0, 72));
  ...
  it.each(["NotFoundError", "NotAllowedError", "NetworkError"])(
    "handles %s and allows retry with freshly fetched GATT objects",
    async (name) => {
  ...
  it("distinguishes a missing GATT service from picker cancellation", async () => {
    vi.mocked(fixture.server.getPrimaryService).mockRejectedValueOnce(
      Object.assign(new Error("missing service"), { name: "NotFoundError" }),
    );
    const { result } = mount();
    await act(() => result.current.connect());
    expect(result.current).toMatchObject({ status: "error", bpm: null });
  ...
  it("guards concurrent clicks and ignores a cancelled pending picker", async () => {
    ...
    act(() => result.current.disconnect());
    await act(async () => {
      pending.resolve(fixture.device);
      await connection;
    });
  ...
  it.each(["person", "reset"])(
    "clears data and disconnects immediately on %s change",
    async (change) => {
      const { result, rerender } = mount();
      await act(() => result.current.connect());
      act(() => fixture.measurement.send(0, 72));
      rerender({
        personId: change === "person" ? "P-02" : "P-01",
  ...
  it.each(["person", "unmount", "disconnect", "demo"])(
    "closes a late GATT connection after %s",
    async (action) => {
      ...
      if (action !== "unmount")
        expect(result.current.mode).toBe(action === "disconnect" ? "device" : "demo");
    },
  );
  ...
  it("does not let an old pending connection disconnect a newer owner of the same device", async () => {
    ...
    await act(async () => {
      oldConnection = result.current.connect();
      await Promise.resolve();
    });
