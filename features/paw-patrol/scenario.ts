...
export const PHASES = [
  {
    ...
    detail: "Units are on their assigned patrols. No active incidents in this scenario.",
  },
  {
    at: 15,
    label: "Signal",
    title: "A signal worth attention.",
    detail: "A scripted camera event reports a possible weapon. Unverified; no injury reported.",
  },
  {
    at: 30,
    label: "Backup",
  ...
  },
  {
    ...
    detail: "The simulated receiving team has the event timeline and sample observations.",
  },
] as const;
export const INCIDENT: Point = [...DEMO_INCIDENT];
export const DESTINATION: Point = [...DEMO_DESTINATION];
export const ROUTE: Point[] =
  VEHICLE_ROUTES.find((route) => route.id === "p-01-transport")?.coordinates ?? [];
export const PEOPLE = [
  {
    id: "P-01",
    name: "Alex Morgan",
...
export function personStatus(id: string, time: number) {
  ...
  if ((id === "P-02" || id === "P-03") && phase >= 2) return phase >= 3 ? "At scene" : "Responding";
  return "On patrol";
}
