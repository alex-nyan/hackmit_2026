...
export type DemoState = {
  time: number;
  running: boolean;
  sceneOverride: SceneReport | null;
  panics: PanicReport[];
  audit: ConsultEvent[];
};
...
export function demoReducer(state: DemoState, action: DemoAction): DemoState {
  switch (action.type) {
    ...
    case "tick": {
      if (!state.running || !Number.isFinite(action.delta) || action.delta < 0) return state;
    ...
    case "scene": {
      ...
      return {
        ...
        audit: [
          ...state.audit,
          {
            id: `scene-${state.audit.length}`,
            at: state.time,
            title: `Scene ${action.status === "cleared" ? "clearance reported" : action.status}`,
            detail:
              action.status === "cleared"
                ? "Demo command authorizes EMS access. No real scene assessment performed."
                : "EMS must stage outside the incident until a clearance report is recorded.",
            source: "Demo operator",
          },
        ],
      };
    ...
    case "panic": {
      ...
      return {
        ...
        time,
        running: false,
        sceneOverride: { status: "unsafe", at: time, source: "Demo operator" },
        panics: [...state.panics, { personId: action.personId, at: time, acknowledgedAt: null }],
        audit: [
          ...state.audit,
          {
            id: `panic-${action.personId}`,
            at: time,
            title: `Panic alert · ${action.personId}`,
            detail:
              "Manual assistance request. Injury and clinical condition are not inferred. Demo paused for scene review.",
            source: "Demo panic button",
          },
        ],
      };
    }
    case "acknowledge": {
      if (!state.panics.some((p) => p.personId === action.personId && p.acknowledgedAt === null))
        return state;
      return {
        ...state,
        panics: state.panics.map((p) =>
          p.personId === action.personId ? { ...p, acknowledgedAt: state.time } : p,
        ),
        audit: [
          ...state.audit,
          {
            id: `ack-${action.personId}`,
            at: state.time,
            title: `Panic acknowledged · ${action.personId}`,
            detail:
              "Command acknowledged the assistance request. Scene access is still governed separately.",
            source: "Demo operator",
          },
        ],
      };
...
export function useScenario() {
  const [clock] = useState(() =>
    createScenarioClock<DemoState, DemoAction>({
