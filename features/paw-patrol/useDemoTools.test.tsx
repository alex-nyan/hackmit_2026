...
describe("workspace-aware demo tools", () => {
  it.each(["command", "officer", "hospital"] as const)(
    "rejects switching away from fixed %s before changing any state",
    async (fixedView) => {
      ...
      await expect(navigation.execute({ view: otherView, personId: "P-02" })).rejects.toThrow(
        "fixed workspace",
      );
      expect(props.setView).not.toHaveBeenCalled();
      expect(props.setSelectedId).not.toHaveBeenCalled();
      expect(tools.get("read_paw_patrol_demo")!.execute({})).toMatchObject({
