...
export function sessionCookie(value: string, request: Request, clear = false) {
  const secure = requestOrigin(request)?.startsWith("https:") ?? false;
...
function requestOrigin(request: Request): string | null {
  try {
    ...
    // destination; production reverse proxies use an explicit canonical origin.
    const configured = process.env.LIVE_PUBLIC_ORIGIN;
    const requestUrl = new URL(request.url);
    const host = request.headers.get("host") ?? requestUrl.host;
    const url = new URL(configured ?? `${requestUrl.protocol}//${host}`);
    if (
      url.username ||
      url.password ||
      url.pathname !== "/" ||
      url.search ||
      url.hash ||
      (configured !== undefined && configured !== url.origin) ||
      (configured === undefined && url.host !== host)
    )
      return null;
    ...
    while (true) {
      ...
      const next = await Promise.race([
        ...
        new Promise<never>((_, reject) => {
          timer = setTimeout(
            () => reject(new Error("request_read_timeout")),
...
export async function login(request: Request) {
  ...
  try {
    ...
    if (typeof body.token !== "string" || !/^[\x21-\x7e]{32,256}$/.test(body.token))
    ...
    return Response.json(session, {
      headers: { ...NO_STORE, "Set-Cookie": sessionCookie(sealSession(body.token), request) },
    });
  } catch (error) {
    return failure(
      error instanceof Error && error.message === "live_session_not_configured"
        ? "live_session_not_configured"
        : "sign_in_unavailable",
      503,
    );
  }
...
export function logout(request: Request) {
  ...
  return Response.json(
    { signed_out: true },
    { headers: { ...NO_STORE, "Set-Cookie": sessionCookie("", request, true) } },
  );
}
...
export async function proxy(request: Request, path: string[]) {
  ...
  const read =
    request.method === "GET" &&
    (session || evidence || (incident && ["state", "events"].includes(path[2])));
  ...
  try {
    ...
    return failure("incident_service_unavailable", 503);
  }
}
