...
describe("Bluetooth heart rate measurement", () => {
  ...
  it("accepts optional energy and multiple RR intervals without treating them as BPM", () => {
    expect(parseHeartRateMeasurement(value(0x18, 81, 8, 0, 4, 0, 9, 0))).toEqual({ kind: "reading", bpm: 81 });
  });
  ...
  it.each([[], [0], [1, 90], [8, 90], [8, 90, 0], [16, 90], [16, 90, 1], [0, 90, 1], [0, 0]])("rejects malformed or unusable packet %j", (...bytes) => {
  ...
  it("handles missing values and DataView byte offsets", () => {
    ...
    expect(parseHeartRateMeasurement(new DataView(buffer, 1, 3))).toEqual({ kind: "reading", bpm: 300 });
  });
  it("treats unreadable buffers as invalid instead of throwing", () => {
    const reading = value(0, 72);
    vi.spyOn(reading, "getUint8").mockImplementation(() => { throw new TypeError("detached"); });
