...
describe("HeartRatePanel", () => {
  it("provides a named region, selected test profile and explicit connection action", () => {
    ...
    const ui = render(<HeartRatePanel connection={source} personId="P-01" personName="Alex Morgan" />);
  ...
  it.each(["requesting", "connecting"] as const)("disables repeated connection attempts while %s", (status) => {
    const source = connection({ mode: "device", status });
    const ui = render(<HeartRatePanel connection={source} personId="P-01" personName="Alex Morgan" />);
    const connect = ui.getByRole("button", { name: status === "requesting" ? "Choose a device…" : "Connecting…" });
  ...
  it.each(["waiting", "receiving", "stale"] as const)("offers disconnect and demo recovery while %s", (status) => {
    const source = connection({ mode: "device", status, deviceName: "HeartCast" });
    const ui = render(<HeartRatePanel connection={source} personId="P-02" personName="Jamie Lee" compact />);
  ...
  it.each(["disconnected", "error"] as const)("offers reconnect after %s without inventing a reading", (status) => {
  ...
  it("labels local receipt time without announcing every updated beat", () => {
    const source = connection({ mode: "device", status: "receiving", bpm: 75, receivedAt: 1_784_000_000_000, message: "Receiving live heart rate." });
    const ui = render(<HeartRatePanel connection={source} personId="P-01" personName="Alex Morgan" />);
    ...
    expect(ui.container.querySelector("time")?.getAttribute("datetime")).toBe(new Date(source.receivedAt!).toISOString());
    expect(ui.getByText(/Received ·/)).toBeTruthy();
    expect(ui.getByText("Live device")).toBeTruthy();
    ui.rerender(<HeartRatePanel connection={{ ...source, bpm: 76, receivedAt: source.receivedAt! + 1_000 }} personId="P-01" personName="Alex Morgan" />);
    expect(status.textContent).toBe("Receiving live heart rate.");
  });

  it("does not render locale-dependent receipt time during server rendering", () => {
    const source = connection({ mode: "device", status: "receiving", receivedAt: 1_784_000_000_000 });
    const html = renderToString(<HeartRatePanel connection={source} personId="P-01" personName="Alex Morgan" />);
  ...
  it("includes keyboard-accessible setup and explicitly explains tab isolation", () => {
    const ui = render(<HeartRatePanel connection={connection()} personId="P-01" personName="Alex Morgan" />);
