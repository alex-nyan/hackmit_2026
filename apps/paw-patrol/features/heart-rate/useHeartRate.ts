...
import { getHeartRateBluetooth, type HeartRateCharacteristic, type HeartRateDevice } from "./bluetooth";
import { parseHeartRateMeasurement } from "./measurement";

export type HeartRateStatus = "idle" | "unsupported" | "requesting" | "connecting" | "waiting" | "receiving" | "stale" | "disconnected" | "error";
export interface HeartRateSample { bpm: number; receivedAt: number }
...
const INITIAL: Snapshot = {
  mode: "demo", status: "idle", bpm: null, receivedAt: null, deviceName: null,
  message: "Demo heart rate. Connect a Bluetooth device to use its readings in this tab.",
  history: [], supported: null,
};
...
class HeartRateStore {
  ...
  private armStaleTimer(attempt: Attempt) {
    this.clearTimer();
    const remaining = Math.max(0, STALE_AFTER_MS - (Date.now() - (attempt.lastUsableAt ?? Date.now())));
    const markStale = () => {
      if (this.isCurrent(attempt)) this.update({
        status: "stale", bpm: null,
  ...
  private release() {
    ...
    if (attempt.device && owners.get(attempt.device) === attempt.token) {
      if (attempt.measurement) {
        try { void attempt.measurement.stopNotifications().catch(() => {}); } catch { /* Already gone. */ }
      }
  ...
  disconnect = () => {
    this.release();
    this.update({ mode: "device", status: "disconnected", bpm: null, receivedAt: null,
      message: "Disconnected. Connect again to resume device readings." });
  };

  useDemo = () => {
    this.release();
    this.update({ ...INITIAL, supported: this.snapshot.supported,
  ...
  connect = async () => {
    ...
    if (!bluetooth) {
      this.update({ supported: false, status: "unsupported",
        message: "Bluetooth requires a supported desktop browser such as Chrome, on HTTPS or localhost." });
      return;
    }
    const attempt: Attempt = { token: Symbol("heart-rate-connection") };
    this.active = attempt;
    this.update({ supported: true, status: "requesting", message: "Choose HeartCast in the browser’s Bluetooth picker." });
    try {
      ...
      attempt.onDisconnect = () => {
        if (!this.isCurrent(attempt)) return;
        this.release();
        this.update({ status: "disconnected", bpm: null, receivedAt: null,
      ...
      this.update({ status: "connecting", deviceName: device.name || "Heart rate device", message: "Connecting to the heart rate service…" });
      const server = await device.gatt.connect();
      if (!this.isCurrent(attempt)) { this.discardLateConnection(attempt); return; }
      ...
      attempt.onMeasurement = () => {
        ...
        if (parsed.kind !== "reading") {
          this.update({ status: "waiting", bpm: null, receivedAt: null,
            message: parsed.kind === "no-contact"
        ...
        this.update({ status: "receiving", ...sample,
      ...
      this.update({ bpm: null, receivedAt: null, status: pickerCancelled ? "disconnected" : "error",
...
export function useHeartRate(personId: string, resetKey = 0) {
  ...
  return { ...snapshot, connect: store.connect, disconnect: store.disconnect, useDemo: store.useDemo };
}
