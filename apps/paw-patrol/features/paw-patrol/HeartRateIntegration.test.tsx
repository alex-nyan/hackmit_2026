...
vi.mock("next/dynamic", () => ({
  default: () => function AnatomyPlaceholder() { return <div>Body viewer</div>; },
}));
...
function receiving(): HeartRate {
  return heartRate({
    ...
    history: [{ bpm: 117, receivedAt: 500 }, { bpm: 123, receivedAt: 1_000 }],
  });
}

beforeEach(() => mockUseHeartRate.mockReturnValue(heartRate()));
...
describe("dashboard heart-rate integration", () => {
  it.each(["dispatch", "officer", "hospital"] as const)(
    ...
    (workspace) => {
      ...
      expect(within(ui.getByLabelText("Selected person heart rate")).getByText(String(sampleHeartRate("P-01", 0)))).toBeTruthy();
    },
  );

  it.each(["dispatch", "officer", "hospital"] as const)(
    "replaces the %s selected-person reading with the received BPM",
    (workspace) => {
  ...
  describe.each(["dispatch", "officer", "hospital"] as const)("%s unavailable measurements", (workspace) => {
    it.each(["waiting", "stale", "disconnected", "error"] as const)(
      "does not present a previous or synthetic measurement as current while %s",
      (status) => {
        // Deliberately retain the prior value: the integration must use status,
        // not just the existence of a number, to label a current reading.
