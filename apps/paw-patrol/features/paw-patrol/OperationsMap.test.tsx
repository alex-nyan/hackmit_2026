...
class TestMap {
  ...
  sources = new Map<string, { setData: ReturnType<typeof vi.fn> }>([["composite", { setData: vi.fn() }]]);
  zoom = 16.1;
  ...
  addLayer = vi.fn((layer: TestLayer) => { this.layers.set(layer.id, layer); });
  addSource = vi.fn((id: string) => { this.sources.set(id, { setData: vi.fn() }); });
  removeLayer = vi.fn((id: string) => { this.layers.get(id)?.onRemove?.(); this.layers.delete(id); });
  constructor(public options: Record<string, unknown>) { mocked.maps.push(this); }
  getStyle() { return { layers: [] }; }
  getSource(id: string) { return this.sources.get(id); }
  getLayer(id: string) { return this.layers.get(id); }
  getZoom() { return this.zoom; }
  getCenter() { return this.center; }
  getBearing() { return -17.6; }
  getPitch() { return 45; }
  isMoving() { return false; }
  isStyleLoaded() { return true; }
  isSourceLoaded() { return true; }
  getCanvas() { return document.createElement("canvas"); }
  project(point: number[]) { return { x: point[0] * 10, y: point[1] * 10 }; }
  on(event: string, handler: Handler) {
    if (!this.handlers.has(event)) this.handlers.set(event, new Set());
    this.handlers.get(event)!.add(handler);
    return this;
  }
  off(event: string, handler: Handler) { this.handlers.get(event)?.delete(handler); return this; }
...
class TestMarker {
  point: number[] = [];
  setLngLat = vi.fn((point: number[]) => { this.point = point; return this; });
  ...
  constructor(public options: { element: HTMLElement }) { mocked.markers.push(this); }
  addTo(map: TestMap) { (map.options.container as HTMLElement).append(this.options.element); return this; }
...
beforeEach(() => {
  ...
  vi.stubGlobal("ResizeObserver", class { observe() {} disconnect = mocked.disconnect; });
  vi.stubGlobal("requestAnimationFrame", vi.fn((callback: FrameRequestCallback) => {
    const id = mocked.nextFrame++;
    mocked.frames.set(id, callback);
    return id;
  }));
  vi.stubGlobal("cancelAnimationFrame", vi.fn((id: number) => { mocked.frames.delete(id); }));
...
describe("patrol map integration", () => {
  it("does not create another map or move the camera just because selection changes", async () => {
    ...
    map.flyTo.mockClear(); map.jumpTo.mockClear(); map.easeTo.mockClear();
    ...
    expect(map.getSource("paw-scenario-route")?.setData).toHaveBeenLastCalledWith(expect.objectContaining({
      features: [expect.objectContaining({ properties: expect.objectContaining({ unit: "P-02" }) })],
    }));
  });
  ...
  it("uses an explicit recenter or area change for camera movement", async () => {
    ...
    expect(map.flyTo).toHaveBeenLastCalledWith(expect.objectContaining({ center: vehicleAt("P-02", 0).point }));
  ...
  it("only follows on request and lets manual map exploration stop following", async () => {
    ...
    expect(map.flyTo).toHaveBeenLastCalledWith(expect.objectContaining({ center: vehicleAt("P-01", 0).point }));
    time = 2;
    frame();
  ...
  it("shares the continuous clock between marker and 3D vehicle positions", async () => {
    ...
    for (const person of PEOPLE) {
      ...
      const marker = mocked.markers.find(value => value.options.element.textContent?.includes(person.id));
  ...
  it("cleans up frame scheduling, marker nodes, listeners and map resources", async () => {
    const { map, unmount } = await mount({ running: true, readClock: () => ({ ...initialDemo, running: true }) });
  ...
  it("handles missing map access and unavailable WebGL without orphan resources", async () => {
    ...
    await waitFor(() => expect(next.getByRole("alert").textContent).toContain("WebGL is unavailable"));
    expect(mocked.maps).toHaveLength(0);
  });

  it("does not leak a duplicate map under Strict Mode", async () => {
    const view = render(<StrictMode><OperationsMap {...props()} /></StrictMode>);
    await waitFor(() => expect(mocked.maps).toHaveLength(1));
    view.unmount();
