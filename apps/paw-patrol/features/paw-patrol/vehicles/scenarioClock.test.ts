...
import { demoReducer, initialDemo, useScenario, type DemoAction, type DemoState } from "../useScenario";
...
function fixture(initialState = initialDemo) {
  ...
  return { ...clock, at: (milliseconds: number) => { timestamp = milliseconds; } };
}
...
describe("shared scenario clock", () => {
  it("reads continuous elapsed time without waiting for a UI tick", () => {
    ...
    clock.at(16); expect(clock.read().time).toBeCloseTo(0.016);
    clock.at(32); expect(clock.read().time).toBeCloseTo(0.032);
    clock.at(250); expect(clock.read().time).toBeCloseTo(0.25);
  });

  it("materializes a control at its exact time and never rewinds on pause", () => {
    const clock = fixture();
    clock.dispatch({ type: "play" });
    clock.at(250); expect(clock.read().time).toBe(0.25);
    clock.at(412); expect(clock.dispatch({ type: "pause" }).time).toBeCloseTo(0.412);
    clock.at(5000); expect(clock.read().time).toBeCloseTo(0.412);
    clock.dispatch({ type: "play" });
    clock.at(5150); expect(clock.read().time).toBeCloseTo(0.562);
  });

  it("does not charge hidden time after the visibility handler pauses", () => {
    const clock = fixture();
    clock.dispatch({ type: "play" });
    clock.at(700); clock.dispatch({ type: "pause" });
    clock.at(60_700); clock.dispatch({ type: "play" });
    expect(clock.read().time).toBeCloseTo(0.7);
    clock.at(60_800); expect(clock.read().time).toBeCloseTo(0.8);
  });

  it("holds an unsafe scene without a rewind from a fractional frame", () => {
    const clock = fixture({ ...initialDemo, time: 59.75, running: true, sceneOverride: { status: "unsafe", at: 45, source: "Demo operator" } });
    clock.at(100); expect(clock.read().time).toBeCloseTo(59.85);
    clock.at(200); const before = clock.read().time;
    clock.at(300); const held = clock.read();
    expect(held.time).toBe(before); expect(held.running).toBe(false);
    clock.at(10_000); expect(clock.read()).toBe(held);
    clock.dispatch({ type: "scene", status: "cleared" });
    clock.dispatch({ type: "play" });
    clock.at(10_100); expect(clock.read().time).toBeCloseTo(60.05);
  });

  it("materializes report timestamps from the same clock", () => {
    const clock = fixture({ ...initialDemo, time: 40, running: true });
    clock.at(155); const state = clock.dispatch({ type: "scene", status: "unsafe" });
  ...
  it("resets, restarts and completes with no stale time accumulation", () => {
    ...
    clock.at(2000); expect(clock.read()).toMatchObject({ time: 90, running: false });
    clock.at(10_000); expect(clock.dispatch({ type: "play" }).time).toBe(0);
    clock.at(10_500); expect(clock.read().time).toBe(0.5);
    clock.dispatch({ type: "reset" });
    clock.at(20_000); expect(clock.read()).toBe(initialDemo);
    clock.dispatch({ type: "play" });
    clock.at(20_200); expect(clock.read().time).toBeCloseTo(0.2);
  });

  it("treats next-stage as one intentional seek and preserves the new anchor", () => {
    const clock = fixture({ ...initialDemo, running: true });
    clock.at(1500); expect(clock.dispatch({ type: "next" }).time).toBe(15);
    clock.at(1700); expect(clock.read().time).toBeCloseTo(15.2);
  });

  it("ignores invalid or backwards monotonic samples", () => {
    const clock = fixture({ ...initialDemo, running: true });
    clock.at(1000); expect(clock.read().time).toBe(1);
    clock.at(500); expect(clock.read().time).toBe(1);
    clock.at(Number.NaN); expect(clock.read().time).toBe(1);
    clock.at(1200); expect(clock.read().time).toBeCloseTo(1.2);
  });
});
