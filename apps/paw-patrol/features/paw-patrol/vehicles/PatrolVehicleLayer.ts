...
function validFrame(vehicle: PatrolVehicleFrame): boolean {
  return Number.isFinite(vehicle.point[0]) && Number.isFinite(vehicle.point[1]) &&
    Math.abs(vehicle.point[0]) <= 180 && Math.abs(vehicle.point[1]) < 85.051129 &&
...
export function createPatrolVehicleLayer(options: PatrolVehicleLayerOptions): CustomLayerInterface {
  ...
  const lostContext = () => { contextLost = true; };
  ...
  return {
    ...
    render(gl, matrix) {
      if (failed || contextLost || gl.isContextLost() || !map || !scene || !camera || !renderer || !resources) return;
      ...
      try {
        ...
        for (const vehicle of options.getVehicles()) {
          ...
          if (!car) {
            ...
            car.group.traverse(object => { object.frustumCulled = false; });
          ...
          car.setEmergency(vehicle.emergency, Number.isFinite(seconds) ? seconds : 0, reducedMotion);
        }
