...
describe("consult workflow safety", () => {
  it.each(["unsafe", "unknown"] as const)("holds EMS for %s even on oversized jumps", status => {
    ...
    expect(state.time).toBe(59); expect(state.running).toBe(false);
  ...
  it("does not rewind or accept pre-incident clearance", () => {
    ...
    const state = demoReducer({ ...initialDemo, time: 59.75, running: true }, { type: "scene", status: "unsafe" });
  ...
  it("uses a distinct scripted clearance in automatic playback", () => {
    ...
    expect(demoReducer({ ...initialDemo, running: true }, { type: "tick", delta: 100 }).time).toBe(90);
  });
  it("keeps panic person-specific, idempotent and distinct from injury", () => {
    ...
    expect(demoReducer({ ...acknowledged, time: 90 }, { type: "play" })).toEqual({ ...initialDemo, running: true });
  });
  it.each([0, 5.25, 14.99, 15, 40])("records panic at %s without advancing any patrol unit", time => {
    const before = PEOPLE.map(person => positionAt(person, time));
    const state = demoReducer({ ...initialDemo, time, running: true }, { type: "panic", personId: "P-02" });
    expect(state).toMatchObject({ time, running: false, sceneOverride: { status: "unsafe", at: time } });
  ...
  it("never invents BP, pulse, or consciousness", () => {
    ...
    expect(record.pulse).toBe("Not assessed"); expect(record.consciousness).toBe("Not assessed");
    expect(validateMist({ ...record, bpMethod: "Cuff", systolic: "120" })).not.toBeNull();
    expect(validateMist({ ...record, bpMethod: "Cuff", systolic: "120", diastolic: "130" })).not.toBeNull();
    ...
    for (const label of ["M — Mechanism", "I — Injuries", "S — Signs", "T — Treatments", "synthetic", "No real patient data"]) expect(text).toContain(label);
  });
});
