...
describe("workspace configuration", () => {
  ...
  it.each(["dispatch", "officer", "hospital"] as const)("isolates %s output", (workspace) => {
    expect(workspaceConfig(workspace, undefined)).toEqual({ workspace, distDir: `.next-${workspace}` });
  ...
  it.each(["command", "admin", "toString", "__proto__", " hospital"])("rejects invalid role %s", (workspace) => {
    expect(() => parseWorkspace(workspace)).toThrow("PAW_PATROL_WORKSPACE");
  });

  it.each(["../build", "/tmp/output", ".next", ".next-hospital", ""])("rejects unsafe or shared output %s", (distDir) => {
